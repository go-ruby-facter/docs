...
`go-ruby-facter/facter` reimplements Ruby's `facter` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-facter/facter` directly, with no Ruby runtime;
...
Because `facter` is pure Go with cgo disabled, it also compiles to **WebAssembly**
